...
from parametricSN.models.models_factory import topModelFactory, baseModelFactory, midModelFactory

from parametricSN.models.sn_fusion_models import sn_FusionModel_1, sn_FusionModel_2, sn_FusionModel_3
...
def run_train(args):
    ...
    params = get_context(args.param_file, full_path=True) #parse params
    ...
    scatteringBaseMul = baseModelFactory( #creat scattering base model
        # architecture=params['scattering']['architecture'],
        architecture=params['scatteringmul']['architecture'],
        J=params['scatteringmul']['J'],
        N=params['dataset']['height'],
        M=params['dataset']['width'],
        second_order=params['scatteringmul']['second_order'],
        initialization=params['scatteringmul']['init_params'],
        seed=params['general']['seed'],
        learnable=params['scatteringmul']['learnable'],
        lr_orientation=params['scatteringmul']['lr_orientation'],
        lr_scattering=params['scatteringmul']['lr_scattering'],
        parameterization=params['scatteringmul']['parameterization'],
        filter_video=params['scatteringmul']['filter_video'],
    )


    if params['scattering']['architecture'] == 'scattering' and params['scatteringmul']['architecture']  == 'scattering':
        viewers = filterVisualizer(scatteringBase, params['general']['seed'])
        lp_init = viewers.littlewood_paley_dekha()
    elif params['scatteringmul']['architecture'] == 'scatteringmul':
        viewers = filterVisualizer(scatteringBaseMul, params['general']['seed'])
        lp_init = viewers.littlewood_paley_dekha()
    else:
        lp_init = None
    ...
    topMul = topModelFactory( #create vit,cnn, mlp, linearlayer, or other
        base=scatteringBaseMul,
    ...
    # fusionModel = sn_FusionModel_1(scatteringBase=scatteringBase, top=top).to(device) #create hybrid model
    # fusionModel = sn_FusionModel_2(scatteringBase=scatteringBase, top=top).to(device) #create hybrid model
    # fusionModel = sn_FusionModel_3(scatteringBaseMul=scatteringBaseMul, topMul=topMul, midMul=midMul).to(device) #create hybrid model
    fusionModel = sn_FusionModel_3(scatteringBaseMul=scatteringBaseMul, topMul=topMul).to(device) #create hybrid model
    # if torch.cuda.device_count() > 1:
    #     fusionModel = nn.DataParallel(fusionModel, device_ids=[0, 1])
    # optimizer = optimizerFactory(hybridModel=hybridModel, params=params)
    optimizer = optimizerFactory(hybridModel=fusionModel, params=params)
    ...
    params['model']['trainable_parameters'] = '%fM' % (countLearnableParams(fusionModel) / 1000000.0)
    ...
    for epoch in range(0, params['model']['epoch']):
        ...
        train_loss, train_accuracy = train(fusionModel, device, train_loader, scheduler, optimizer,
        ...
        if epoch % params['model']['step_test'] == 0 or epoch == params['model']['epoch'] -1: #check test accuracy
            ...
            accuracy, test_loss = test(fusionModel, device, test_loader)
    ...
    log_mlflow(
        ...
        params=params, model=fusionModel, test_acc=np.array(test_acc).round(2),
...
def main():
    ...
    #dataset
    subparser.add_argument("--dataset-name", "-dname", type=str, choices=['cifar', 'x-ray', 'imagenet-tiny', 'imagenet-1k'])
    ...
    # subparser.add_argument("--model-name", "-mname", type=str, choices=['vitonly', 'vit', 'cnn', 'mlp', 'linear_layer', 'resnet50'])
    subparser.add_argument("--model-name", "-mname", type=str, choices=['mulvit', 'vitonly', 'vit', 'cnn', 'mlp', 'linear_layer', 'resnet50'])
    ...
    subparser.add_argument('--param_file', "-pf", type=str, default='/xxx/conf/parameters_cpst.yml',
                        help="YML Parameter File Name")
