...
from parametricSN.data_loading.image_loader import img_getDataloaders
from parametricSN.data_loading.imagetiny_loader import imgtiny_getDataloaders
...
def datasetFactory(params, dataDir):
    ...
    elif params['dataset']['name'].lower() == "imagenet-tiny":
        return imgtiny_getDataloaders(
            trainSampleNum=params['dataset']['train_sample_num'], valSampleNum=params['dataset']['test_sample_num'],
            trainBatchSize=params['dataset']['train_batch_size'], valBatchSize=params['dataset']['test_batch_size'],
            trainAugmentation=params['dataset']['augment'], height=params['dataset']['height'],
            width=params['dataset']['width'], dataDir=dataDir
        )
    elif params['dataset']['name'].lower() == "imagenet-1k":
        return img_getDataloaders(
            trainSampleNum=params['dataset']['train_sample_num'], valSampleNum=params['dataset']['test_sample_num'],
            trainBatchSize=params['dataset']['train_batch_size'], valBatchSize=params['dataset']['test_batch_size'],
            trainAugmentation=params['dataset']['augment'], height=params['dataset']['height'],
            width=params['dataset']['width'], dataDir=dataDir
