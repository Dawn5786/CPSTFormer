...
def create_filters_params(J, L, is_scattering_dif, equivariant=False):
    ...
    xis = torch.tensor(xis, dtype=torch.float32)                        # by pi, j
    sigmas = torch.tensor(sigmas, dtype=torch.float32)                  # by j
    slants = torch.tensor(slants, dtype=torch.float32)                  # by L
    orientations = torch.tensor(orientations, dtype=torch.float32)      # by L, pi
