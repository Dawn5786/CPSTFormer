...
from kymatio.torch import Scattering2D, Scattering2DMul
...
# class sn_MultiScatteringBase(Scattering2D):
class sn_MultiScatteringBase(Scattering2DMul):
    """A learnable Multi-batch scattering nn.module
    parameters:
        learnable -- should the filters be learnable parameters of this model
        J -- scale of scattering (maybe 2,3,4 for now)
        N -- height of the input image
        M -- width of the input image
    """
    ...
    def __init__(self, J, N, M, second_order, initialization, seed,
        ...
        #super(sn_ScatteringBase, self).__init__(J=J, shape=(M, N))
        super(sn_MultiScatteringBase, self).__init__(J=J, shape=(M, N))
        ...
        if second_order:
            ...
            self.n_coefficients = L * L * J * (J - 1) // 2 + 1 + L * J * 2 
        ...
        def reshape_hook(self, x, S):
            S0 = S[0]
            S0 = S0.reshape(S0.size(0), S0.size(1)*S0.size(2), S0.size(3), S0.size(4))
            S1 = S[1]
            S1 = S1.reshape(S1.size(0),  S1.size(1)*S1.size(2), S1.size(3), S1.size(4))
            S2 = S[2]
            S2 = S2.reshape(S2.size(0), S2.size(1)*S2.size(2), S2.size(3), S2.size(4))
            return S0, S1, S2
